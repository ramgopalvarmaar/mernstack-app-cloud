...
const linksboard = (state = { lists: [] }, action) => {
  switch (action.type) {
    case "LINKS_ADD_LIST": {
      const { listId } = action.payload;
      return { lists: [...state.lists, listId] };
    }
    case "LINKS_MOVE_LIST": {
      const { oldListIndex, newListIndex } = action.payload;
      const newLists = Array.from(state.lists);
      const [removedList] = newLists.splice(oldListIndex, 1);
      newLists.splice(newListIndex, 0, removedList);
...
const linkslistsById = (state = {}, action) => {
  switch (action.type) {
    case "LINKS_ADD_LIST": {
      const { listId, listTitle } = action.payload;
      return {
...
const linkscardsById = (state = {}, action) => {
  switch (action.type) {
    case "LINKS_ADD_CARD": {
      const { cardText, cardId } = action.payload;
      return { ...state, [cardId]: { text: cardText, _id: cardId } };
    }
    case "LINKS_CHANGE_CARD_TEXT": {
      const { cardText, cardId } = action.payload;
      return { ...state, [cardId]: { ...state[cardId], text: cardText } };
    }
    case "LINKS_DELETE_CARD": {
